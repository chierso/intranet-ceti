...
class abm_record_model extends CI_Model {
	...
	function update_notas($pIdAlumno, $pGrado, $pSeccion, $pCurso, $pBimester, $pNota) {
		...
		$idRecord = $this ->get_id_record($pIdAlumno, $pGrado, $pSeccion, $pCurso , date('Y'));
	...
	function get_id_record($pAlumno, $pGrado, $pSeccion, $pCurso,$pYear) {
		$this -> db -> select('rc.id_record');
		$this -> db -> from('tbl_registration rg, tbl_record rc');
		$this -> db -> where('rg.id_alumn = ' . $pAlumno . ' AND rg.grade = ' . $pGrado . ' AND rg.section = "' . $pSeccion . '" AND rg.year = "' . $pYear . '" AND rc.id_subject = ' . $pCurso . ' AND rc.id_registration = rg.id_registration');
		$query = $this -> db -> get();
		$row = $query -> row();
		if ($query -> num_rows() > 0) {
