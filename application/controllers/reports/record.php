<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	public function reporteAlumnoParametroPDF($pAlumno)
	{
		...
		if($pTipoUsuario == 'dir') $pTipoUsuario = "Director";
		if($pTipoUsuario == 'doc') $pTipoUsuario = "Docente";
		if($pTipoUsuario == 'alum') $pTipoUsuario = "Alumno";
		$this->load->model('abm/abm_record_model');
		//$data = $this->abm_record_model->listar_notas($pAlumno);
		$data = $this->abm_record_model->listar_notas($pAlumno);
		...
		foreach ($data as $row) {
			...
			'<td align="left" width="42%">'.$row->name.'</td>'.
			'<td width="12%">'.$row->N1_average.'</td>'.
			'<td width="12%">'.$row->N2_average.'</td>'.
			'<td width="12%">'.$row->N3_average.'</td>'.
			'<td width="12%">'.$row->N4_average.'</td>
